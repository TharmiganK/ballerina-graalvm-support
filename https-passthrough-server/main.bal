...
listener http:Listener serverEP = new(9090, httpVersion = http:HTTP_1_1, secureSocket = {
    key: {
        path: KEYSTORE_PATH,
        password: password
    }
...
service on serverEP {
    ...
    resource function 'default [string ...path](http:Request req) returns string|error {
        http:Client clientEP = check new("https://localhost:8080", httpVersion = http:HTTP_1_1, secureSocket = {
            cert: {
